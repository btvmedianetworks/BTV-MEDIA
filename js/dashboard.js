...
const state = {
  ...
  language: 'telugu',
  cardLanguage: 'telugu',
  // Title font selection (Mandali for Telugu, Roboto for English)
  titleFont: 'Mandali',
  // Description font selection (Mandali for Telugu, Roboto for English)
  descriptionFont: 'Mandali',
  titleSize: 48,
  descriptionSize: 30,
  gap: 24,
  autoFit: true,
...
function setCardLanguage(lang) {
  ...
  if (nextLang === 'english') {
    ...
    state.titleFont = 'Mandali';
    state.descriptionFont = 'Mandali';
  }
  syncDisplayValues();
...
async function renderCard(props = {}) {
  ...
  const titleFont = cardData.titleFont || (isEnglishMode ? 'Roboto' : 'Mandali');
  const descriptionFont = cardData.descriptionFont || (isEnglishMode ? 'Roboto' : 'Mandali');

  const titleFontFamily = getTitleFontFamily(titleFont);
  const descFontFamily = getDescriptionFontFamily(descriptionFont);

  const titleFontSize = Math.max(28, Math.min(96, Number(cardData.titleSize !== undefined ? cardData.titleSize : state.titleSize) || 48));
  ...
  // =====================================================
  // Title rendering (English: Roboto Bold, Telugu: Mandali / Noto Sans Telugu Bold)
  ctx.save();
...
function calculateCardContentHeight(titleSize, descSize, titleText = state.title, descText = state.description, gap = state.gap, titleFont = state.titleFont, descFont = state.descriptionFont) {
  ...
  const titleWeight = isTeluguFont(titleFont) ? '700' : '800';
  measureCtx.font = `${titleWeight} ${titleSize}px ${titleFontFamily}`;
  const titleLines = wrapText(measureCtx, titleText || 'Breaking update from the newsroom', titleAreaWidth);
  const titlePaddingTop = Math.max(12, Math.round(titleSize * 0.12));
  const titleLineHeight = isTeluguFont(titleFont)
    ? Math.round(titleSize * 1.35)
  ...
  const descLineHeight = Math.round(descSize * 1.38);
  const descTotalHeight = descLines.length * descLineHeight;
...
function updateSizeButtonStates() {
  const currentTitleSize = Number(state.titleSize) || 48;
  const currentDescSize = Number(state.descriptionSize) || 30;

  // Title buttons (+ and -) bounded to practical range for 9:16 card (28px - 96px)
  ...
  if (titlePlusBtn) {
    ...
    if (!canIncrease) {
      titlePlusBtn.setAttribute('title', nextTitleSize > 96 ? 'Maximum title size is 96px' : 'Title cannot be enlarged further without exceeding the card');
    } else {
      titlePlusBtn.removeAttribute('title');
    }
  }
  if (titleMinusBtn) {
    titleMinusBtn.disabled = currentTitleSize <= 28;
    if (currentTitleSize <= 28) {
...
function handleStepChange(target, stepValue) {
  ...
  if (target === 'titleSize') {
    const currentSize = Number(state.titleSize) || 48;
    const targetSize = currentSize + step;
    if (step > 0) {
      if (targetSize > 96 || !canTextFit(targetSize, state.descriptionSize)) {
        updateSizeButtonStates();
        return; // STOP: cannot exceed 96px or overflow available card content space
      }
    } else if (step < 0) {
      if (targetSize < 28) {
        updateSizeButtonStates();
        return; // STOP: cannot go below 28px
      }
    }
    state.titleSize = Math.max(28, Math.min(96, targetSize));
...
async function triggerPublish(selectedCategories = ['News']) {
  ...
  try {
    ...
    const payload = {
      ...
      // Title font selection
      titleFont: state.titleFont || (state.language === 'english' ? 'Roboto' : 'Mandali'),
      // Description font selection
      descriptionFont: state.descriptionFont || (state.language === 'english' ? 'Roboto' : 'Mandali'),
      categories,
...
function applyCardFromStorage(post) {
  ...
  // Title font selection
  state.titleFont = post.titleFont || (state.language === 'english' ? 'Roboto' : 'Mandali');
  // Description font selection
  state.descriptionFont = post.descriptionFont || (state.language === 'english' ? 'Roboto' : 'Mandali');
